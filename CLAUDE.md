...
4. **Self-contained.** Postgres, Directus and the frontend are the only services. No
   ...
   | `www.riehenevents.ch`                       | Riehens offizieller Veranstaltungskalender — eine eigene Domain und ein eigenes CMS, aber der Kalender DER GEMEINDE, und darum `art: gemeinde` wie die neun anderen. Gelesen wird die Tuer, die die Seite selbst ruft (`/de/api/event` mit `dateFrom`, `dateTo`, `limit`, `offset`), TAG FUER TAG ueber das Fenster: eine Woche am Stueck verliert gemessen 6 von 14 Anlaessen. `robots.txt` ist das Drupal-Standardblatt und sperrt `/admin/`, `/core/`, `/search/`, `/user/…` — die Datentuer nicht. Der Crawler bleibt hier aussen vor | `shared/veranstaltung/drupal.ts`                 |
   ...
   **Since 22 September 2026 every article can carry a `termin`** — when it
   counts for the reader, and on which days the Dorfkönig should bring it
   (`redaktion/termin.ts`, contract version 1.4.0, proposed by the Dorfkönig
   on 21 September). `{ideal, ende, auftritte}`: the ideal day (the
   registration deadline, the first day, the day of the closure), the last day
   the article still makes sense, and the reading days in the briefing —
   at most five, none after `ende`; without a list the Dorfkönig falls back to
   its old rule, once on the workday before `ideal`. The newsroom's words:
   hand over the day; for an IMPORTANT event several appearances, the first
   announcement immediately, then the day itself, for a long exhibition also
   ...
   the way out in `endpoints/api/projektion.ts` — an article lies on the desk
   for days before it goes out, and a past reading day is no appearance. The
   proposal is kept twice (`termin_vorschlag`, `wichtig_vorschlag`) beside
   what the newsroom sets (`termin`, `wichtig`, `POST /redaktion/meldungen/:id/termin`,
   editable after publication because the Dorfkönig re-reads `/artikel/{id}`
   daily), and the deviation is the learning signal: `redaktion/wichtigkeit.ts`
   reads the last twenty verdicts of the desk — taste is the newsroom's, so
   across all municipalities, deviations first, the cap declared — into the
   writer's user turn. No rule is distilled from a flipped switch yet: the
   decision learner writes `stufe: sichtung` rules, and importance is a
   `text`-level judgement; a rule for it is typed by hand in „Gelerntes" until
   ...
   **The calendars are a LIST per municipality (`veranstaltungsquellen`), not
   ...
   read it.
   **Ein Kalender muss nicht auf der Gemeindeseite liegen.** Riehens
   offizieller Kalender hat eine eigene Domain und ein eigenes CMS, und die
   Redaktion hat am 20. September 2026 entschieden, dass er trotzdem der
   Kalender der Gemeinde ist: `art: gemeinde`, gelesen wie die neun anderen,
   nicht eine Fremdplattform, die stillsteht. Seine Agenda baut ihre Liste
   erst im Browser — ihr rohes HTML traegt NULL Eintraege, ein Listen-Parser
   haette dort fuer immer „keine Eintraege" gemeldet. Also fragt der Leser
   die Tuer, die die Seite selbst ruft (`shared/veranstaltung/drupal.ts`),
   und `ueberDatentuer` ist, was die beiden Wege auseinanderhaelt. **Diese
   Tuer muss TAG FUER TAG befragt werden, und das ist gemessen, nicht
   angenommen:** eine Woche am Stueck liefert 8 Anlaesse, dieselbe Woche Tag
   fuer Tag 14, und die 8 sind eine echte Teilmenge; `offset` zaehlt
   VORKOMMEN, waehrend die Antwort Serien liefert, so dass Seiten einander
   ueberlappen und zugleich Luecken lassen (`limit=100` in zwei Abrufen: 58
   Serien, `limit=20` in neun Abrufen ueber denselben Bereich: 116). Zu
   blaettern hiesse hier, eine stille Stichprobe zu nehmen. Das Datum eines
   Eintrags ist der Tag, nach dem GEFRAGT wurde — die Antwort schreibt „So. 20. September" ohne Jahr —, und die Serie steckt in der id (`9838_486` ist
   der 486. Tag derselben Ausstellung), so dass die Gruppierung ohne Raten
   auskommt.
...
| a calendar that answers only through a data door                | eine eigene Familie in `shared/veranstaltung/` plus ein Fingerabdruck in `shared/gemeindeseite/erkennung.ts` und ein Eintrag in `TUER_VORLAGEN` — `leseUebersicht` fragt sie dann, statt ihr HTML zu parsen. Muster: `drupal.ts`                                                   |
